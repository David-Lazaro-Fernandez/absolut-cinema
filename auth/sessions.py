"""Sesiones (`session`): una fila por cookie emitida. El valor de la cookie nunca se guarda, solo su sha256."""
from datetime import datetime, timedelta, timezone

from . import security
from .db import cursor, one, transaction

_USER = "a.id, a.email, a.name, a.role AS role, a.active"
...
def create(conn, user_id, ip=None, user_agent=None, now=None):
    ...
    with cursor(conn) as cur:
        cur.execute("""INSERT INTO session (token_hash, account_id, created_at, last_seen_at, expires_at, ip, user_agent)
                       VALUES (?, ?, ?, ?, ?, ?, ?)""",
...
def current_session(conn, raw, now=None):
    ...
    row = one(conn, f"""SELECT {_USER}, s.last_seen_at
                        ...
                        WHERE s.token_hash = ? AND s.revoked_at IS NULL AND s.expires_at > ? AND a.active""",
              (token_hash, now))
    if row is None:
        return None
    if now - row["last_seen_at"] > timedelta(minutes=security.SESSION_TOUCH_MINUTES):
        with transaction(conn):
            with cursor(conn) as cur:
                cur.execute("UPDATE session SET last_seen_at = ?, expires_at = ? WHERE token_hash = ?",
...
def revoke(conn, raw, now=None):
    ...
    row = one(conn, """UPDATE session SET revoked_at = ?
                       WHERE token_hash = ? AND revoked_at IS NULL RETURNING account_id""",
...
def revoke_all(conn, user_id, now=None):
    with cursor(conn) as cur:
        cur.execute("UPDATE session SET revoked_at = ? WHERE account_id = ? AND revoked_at IS NULL",
                    (now or datetime.now(timezone.utc), user_id))


def prune(conn, keep_days=90, now=None):
    ...
    with transaction(conn):
        with cursor(conn) as cur:
            cur.execute("DELETE FROM session WHERE coalesce(revoked_at, expires_at) < ?", (cutoff,))
            n_sessions = cur.rowcount
            cur.execute("DELETE FROM token WHERE coalesce(used_at, expires_at) < ?", (cutoff,))
