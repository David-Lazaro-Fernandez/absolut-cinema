"""Explorador de datos: tablas curadas de `snapshots.db` con filtros en la barra lateral, orden y búsqueda de la
propia tabla, y descarga en CSV. La consulta vive en `analytics/datasets.py`; aquí solo se arman los controles que
declara `analytics.datasets.DATASETS` para cada conjunto y se pinta el resultado."""
...
key = st.sidebar.selectbox(DATA_TEXT["dataset"], options=list(analytics.datasets.DATASETS), format_func=DATASET_LABEL.get,
                           key="dataset", label_visibility="collapsed")
filters = analytics.datasets.DATASETS[key]["filters"]
...
if "category" in filters:
    cats = load_dataset("categories", dataset=key)
...
df = load_dataset(key, **kwargs)

if df.empty:
    st.info(DATA_TEXT["empty"])
    st.stop()

if len(df) >= analytics.datasets.MAX_ROWS:
    st.warning(DATA_TEXT["truncated"].format(n=f"{analytics.datasets.MAX_ROWS:,}"))
else:
