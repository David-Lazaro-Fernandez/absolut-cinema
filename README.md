...
- `deploy/`: systemd, respaldo y Caddy para correr todo en un servidor (`deploy/README.md`); `deploy/postgres/` y
  `deploy/docker-compose.dev.yml` para probar el esquema del archivo histórico en Postgres local (`make pg-up pg-schema`).
...
Programación con launchd mientras no esté en el servidor: cuatro agentes (cartelera tres veces al día, planos cada hora,
diario, delivery) que ejecutan targets de `make`. El repo debe estar fuera de `~/Documents`, `~/Desktop` y `~/Downloads`:
